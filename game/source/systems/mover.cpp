...
static std::map<HexType, Movability> get_hex_table(UnitType type) noexcept {
    switch (type) {
        ...
        default:
            engine_assert(false, "Unknown unit type.");
            return {};
    }
}

static std::map<RiverType, Movability> get_river_table(UnitType type) noexcept {
    switch (type) {
        case UnitType::heavy:
            return {
                {RiverType::small, 3},
...
MovementSystem::MovementSystem(const GameState& state) noexcept
...
bool MovementSystem::is_moving() const noexcept { return _target_component != nullptr; }
