...
void ConsoleWindow::execute_command(std::string cmd) {
    ...
    std::remove(_history.begin(), _history.end(), cmd);
...
void ConsoleWindow::show(bool* p_open) {
    ...
    if (ImGui::SmallButton("Add Dummy Text")) {
        add_line("some text");
        add_line("some more text");
        add_line("display very important message here!");
    }
    ImGui::SameLine();
    if (ImGui::SmallButton("Add Dummy Error")) {
        add_line("[error] something went wrong");
    }
    ImGui::SameLine();
...
and history. A more elaborate implementation may want to store entries along
with extra data such as timestamp, emitter, etc."); ImGui::TextWrapped( "Enter
'HELP' for help, press TAB to use text completion.");
